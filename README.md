...
6. **Local history** — finished reviews are saved in your browser (localStorage — no accounts, nothing server-side). Reopen a past review, contest it again, and track open invalidations: every verdict names the condition that should kill the trade, and re-checking it against fresh sources is one click. Back up the whole history to a JSON file and import it into another browser — still no accounts.
